...
class ApostaController extends Controller
{
    public function cadastro(Request $request)
    {
        ...
        $listaJogos = $request->input('aposta');

        $teamFrequency = array_count_values($listaJogos);

        $equipeList = [];

        foreach ($listaJogos as $key => $value) {
            $jogo = Jogo::find($key);
            if ($jogo == null) {
            ...
            $equipeEscolhido = new EquipeEscolhido();
            $equipeEscolhido->jogo_id = $key;
            $equipeEscolhido->equipe_id = $value;
            array_push($equipeList, $equipeEscolhido);
        ...
        $aposta->equipeEscolhidos()->saveMany($equipeList);
    ...
    public function buscar(Request $request) {
        ...
        if ($request->query('gol_casa') != null) {
            $rodadas = $rodadas->where('gol_casa', $request->query('gol_casa'));
        }
